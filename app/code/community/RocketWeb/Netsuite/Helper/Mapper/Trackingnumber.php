...
class RocketWeb_Netsuite_Helper_Mapper_Trackingnumber extends RocketWeb_Netsuite_Helper_Mapper {
    ...
    public function getNormalizedTrackingNumberData(ItemFulfillment $netsuiteShipment) {
        ...
        if (isset ($netsuiteShipment->customFieldList) && is_array($netsuiteShipment->customFieldList->customField)) {
            foreach ($netsuiteShipment->customFieldList->customField as $netsuitePackage) {
                if ($netsuitePackage->internalId == 'custbody_do_trackingnumber') {
                    $number = !empty ($netsuitePackage->value) ? $netsuitePackage->value : ' ';
                    $description = $netsuiteShipment->shipMethod->name ? $netsuiteShipment->shipMethod->name : 'Tracking Number';
                    $trackingNumbers[] = array ('number' => $number, 'description' => $description);
                }
            }
        }
