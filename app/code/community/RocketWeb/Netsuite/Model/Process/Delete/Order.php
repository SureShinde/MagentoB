...
class RocketWeb_Netsuite_Model_Process_Delete_Order extends RocketWeb_Netsuite_Model_Process_Delete_Abstract {
    public function processDeleteOperation(DeletedRecord $record) {
        if(!$record->record->internalId) {
            return;
        }
        $shipmentCollection = Mage::getModel('sales/order')->getCollection();
