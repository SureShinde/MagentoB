...
require dirname(__FILE__).DS."NSconfig.php";
...
class NSPHPClient {
    ...
    protected function __construct($wsdl=null, $options=array()) {
        ...
        /* [pgrigoruta] [customization] Adding timeout to SOAP calls. According to http://www.darqbyte.com/2009/10/21/timing-out-php-soap-calls/
        the php.ini value wont always work. Original line commented out.
        */
        //$this->client = new SoapClient($wsdl, $options);
        $this->client = new SoapClientWithTimeout($wsdl, $options);
        $timeout = (int)Mage::getStoreConfig('rocketweb_netsuite/general/soap_request_timeout');
        if($timeout) {
            $this->client->__setTimeout($timeout);
        }
        //customization end
    ...
    protected function makeSoapCall($operation, $parameter) {
        ...
        //[pgrigoruta] [customziation] - catch the possible exception, log it and continue. The point of doing this here is to allow logging the api call below
        ...
        //[pgrigoruta] [customization] - We log in a table, respecting the debug flag instead of a directory.
        if(Mage::getStoreConfig('rocketweb_netsuite/developer/enabled')) {
        	
        	$logItem = Mage::getModel('rocketweb_netsuite/apilogitem');
        	...
        	$logItem->setRequest($xml_string);
        	
        	$data = $this->client->__getLastResponse();
        	$logItem->setResponse($data);
        	
        	$logItem->setCallDate(gmdate('Y-m-d H:i:s'));
        	$logItem->save();
        ...
        */
        ...
        if(!$exceptionRaised) return $response;
        else return null;
