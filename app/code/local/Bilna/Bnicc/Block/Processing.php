<?php
/**
 * Description of Bilna_Bnicc_Block_Processing
 *
...
class Bilna_Bnicc_Block_Processing extends Mage_Payment_Block_Info {
    protected $_code = 'bnicc';
    protected $_file = 'redirect';
    ...
    public function getTokenVeritrans($orderId) {
        ...
        $request->promo_bins = $this->getPromoBins();
    ...
    protected function getPromoBinsSingle($entityId) {
        return Mage::getModel('bnicc/bnicc')->getCCBinsByEntityId($entityId);
    }
    
    protected function getPromoBins() {
        $issuer = (string) Mage::getStoreConfig('payment/' . $this->_code . '/issuer');
        $promoBins = Mage::getModel('bnicc/bincode')
            ->getCollection()
