    ...
    public function getElementHtml()
    {
        ...
        foreach ($collection as $key => $value) {
            $inject .= 'arrayRow.add({
                            ...
                            "helper_message":"'.Mage::helper('core')->escapeHtml($value['helper_message']).'",
                            "order":"'.$value['order'].'",
                ...
                var arrayRow = {
                    template : new Template(
                        \'<tr id="row-#{row_counter}">\'+
                            ...
                            \'<td><input name="value[#{row_counter}][message]" value="#{helper_message}" class="input-text required-option" type="text" style="width:50px;text-align:center;"></td>\'+
