...
class AW_Points_Model_Rule_Condition_Product_Subselect extends AW_Points_Model_Rule_Condition_Product_Combine {
    ...
    public function asHtml() {
        $html = $this->getTypeElement()->getHtml() .
                Mage::helper('points')->__("If %s %s %s for a subselection of items in cart matching %s of these conditions:", $this->getAttributeElement()->getHtml(), $this->getOperatorElement()->getHtml(), $this->getValueElement()->getHtml(), $this->getAggregatorElement()->getHtml()
        );
        if ($this->getId() != '1') {
    ...
    public function validateOnCheck(Varien_Object $object, $pointToBeRewarded) {
        ...
        $total = 0;
        foreach ($object->getAllItems() as $item) {
            if (parent::validateOnCheck($item)) {
                $total += $item->getData($attr);
            }
        }
        return $this->validateAttribute($total);
