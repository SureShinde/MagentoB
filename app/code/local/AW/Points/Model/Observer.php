...
class AW_Points_Model_Observer extends Mage_Core_Block_Abstract
{
    ...
    protected function _addPointsForRules($observer)
    {
        ...
        foreach ($ruleCollection as $rule) {
            if ($rule->checkRule($quote)) {
                ...
                }else if($rule->getApplyOn() == "by_fixed"){
                	$extraPointToBeAdd = $rule->getPointsChange();
                }else if($rule->getApplyOn() == "by_percent"){
                	$extraPointToBeAdd = ($quote->getSubTotal() * $rule->getPointsChange()) / 100;
                	//UNMARK IF ONLY DISCOUNT PRODUCT THAT MATCH THE RULE
                	//                 	}else{
                	//         				if(is_int($rule->getPointByRule($this->_quote)) || is_float($rule->getPointByRule($this->_quote))){
                	//                 			$extraPointToBeAdd = $rule->getPointByRule($this->_quote);
                	//         				}else{
                	//                 			$extraPointToBeAdd = $rule->getPointsChange();
                	//         				}
                }
                ...
                $extraPointToBeAdd = $extraPointToBeAdd - ($extraPointToBeAdd % 500);

                Mage::getModel('points/api')->addTransaction(
                    $extraPointToBeAdd, 'order_invoiced', $quote->getCustomer(), $order, array('order_increment_id' => $order->getIncrementId()), array('notice' => Mage::helper('points')->__('Rule #%s, %s', $rule->getId(), $rule->getName()),
    ...
    protected function _hasCustomerBoughtThisProduct($_customerId, $_productIdToVerify)
    {
        ...
        //RULE FOR CUSTOMER MUST ORDERED BEFORE ADD REVIEW
        $collectionOfOrders = Mage::getModel('sales/order_item')
                ->getCollection();
        //RULE FOR SPECIFIC PRODUCT THAT CUSTOMER MUST BOUGHT FIRST
//                 ->addAttributeToFilter('main_table.product_id', $_productIdToVerify);
        //ADD CONDITION: ORDER WITH SPECIFIC CUSTOMER
        $collectionOfOrders->getSelect()->join(	array('or' => 'sales_flat_order'), 
        										"or.entity_id = main_table.order_id AND or.customer_id = ".$_customerId, 
        										array());
        //ADD CONDITION: ORDER MUST BE PAYED BEFORE THE REVIEW
        $collectionOfOrders->getSelect()->join(	array('orhis' => 'sales_flat_order_status_history'), 
        										"orhis.parent_id = main_table.order_id AND orhis.status = 'processing'", 
        										array());
        $collectionOfOrders->getSelect()->group(array('main_table.product_id'));
        
        if ($collectionOfOrders->count() > 0){
        	$result = true;
        }

        //POINT REWARD ORIGINAL CODE
//         $product = Mage::getModel('catalog/product')
