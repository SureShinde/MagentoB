...
class AW_Collpur_Model_Deal extends Mage_Core_Model_Abstract {
    ...
    public function processOrderItem($orderItem, $errors=true) {
        ...
        $maximumAllowedPurchases = $this->getData('maximum_allowed_purchases');

        if ($errors) {
            if ($maximumAllowedPurchases && $afterPurchaseQty > $maximumAllowedPurchases) {
                throw new Mage_Core_Exception(Mage::helper('collpur')->__('Qty is not available, available qty: %s', $maximumAllowedPurchases - $this->getPurchasesCount() + $purchase->getData('qty_purchased')));
            }
            if ($this->getData('auto_close') && $afterPurchaseQty > $this->getData('qty_to_reach_deal')) {
                throw new Mage_Core_Exception(Mage::helper('collpur')->__('Qty is not available, available qty: %s', $this->getData('qty_to_reach_deal') - $this->getPurchasesCount() + $purchase->getData('qty_purchased')));
            }
        }
