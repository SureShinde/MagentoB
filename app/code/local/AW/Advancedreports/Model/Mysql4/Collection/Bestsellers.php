...
class AW_Advancedreports_Model_Mysql4_Collection_Bestsellers extends Mage_Catalog_Model_Resource_Eav_Mysql4_Product_Collection
{
    ...
    public function setDateFilter($from, $to)
    {
        ...
        $this->getSelect()->where("orders.{$filterField} >= ?", $from)
    ...
    public function setState()
    {
        if ($this->_helper()->checkSalesVersion('0.9.56')) {
            ...
            $this->getSelect()
                ...
                ->join(array('val' => $entityValues), "attr.attribute_id = val.attribute_id AND " . $this->_helper()->getSql()->getProcessStates(), array())
    ...
    public function addOrderItems($limit = 10, $skuType = AW_Advancedreports_Model_System_Config_Source_Skutype::SKUTYPE_SIMPLE)
    {
        ...
        if ($skuType == AW_Advancedreports_Model_System_Config_Source_Skutype::SKUTYPE_SIMPLE) {
            $this->getSelect()
                ->join(array('item' => $itemTable), "(item.product_id = e.entity_id AND ((item.parent_item_id IS NULL AND item.product_type NOT IN ({$notSimple})) OR (item.parent_item_id IS NOT NULL AND item.product_type NOT IN ({$notSimple}))))", array('product_id' => 'product_id', 'sum_qty' => 'SUM(item.qty_ordered)', 'sum_total' => 'SUM(IF(item.base_row_total>0,item.base_row_total, item2.base_row_total))', 'name' => 'name', 'sku' => 'sku'))
                ->joinLeft(array('item2' => $itemTable), "(item.parent_item_id = item2.item_id AND item2.product_type = 'configurable')", array());
        } else {
            $productTable = $this->_helper()->getSql()->getTable('catalog_product_entity');
            $this->getSelect()
                ->join(array('item' => $itemTable), "(item.product_id = e.entity_id AND item.parent_item_id IS NULL)", array('product_id' => 'product_id', 'sum_qty' => 'SUM(item.qty_ordered)', 'sum_total' => 'SUM(item.base_row_total)', 'name' => 'name', 'sku' => 'realP.sku'))
                ->joinLeft(array('realP' => $productTable), "item.product_id = realP.entity_id", array());
        }
