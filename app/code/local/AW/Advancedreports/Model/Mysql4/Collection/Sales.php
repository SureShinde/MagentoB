...
class AW_Advancedreports_Model_Mysql4_Collection_Sales extends AW_Advancedreports_Model_Mysql4_Collection_Abstract
{
    ...
    public function reInitSelect()
    {
        ...
        # total
            ->columns(array('base_row_xtotal_incl_tax' => "( IFNULL(item2.qty_ordered, item.qty_ordered) * IFNULL(item2.base_price, item.base_price) - ABS( IFNULL(item2.base_discount_amount,item.base_discount_amount) ) + IFNULL(item2.base_tax_amount, item.base_tax_amount) - ABS(COALESCE(awpto.points_to_money,0)/child_soi.total_item) )"))
            ->columns(array('base_row_xtotal' => "( IFNULL(item2.qty_ordered, item.qty_ordered) * IFNULL(item2.base_price, item.base_price) - ABS( IFNULL( item2.base_discount_amount, item.base_discount_amount ) ) - ABS(COALESCE(awpto.points_to_money,0)/child_soi.total_item) )"))
        # invoiced
            ->columns(array('base_row_xinvoiced' => "IF(( IFNULL(item2.qty_invoiced, item.qty_invoiced) * IFNULL(item2.base_price, item.base_price) - ABS(IFNULL(item2.base_discount_amount, item.base_discount_amount) ) ) < 0, 0, ( IFNULL(item2.qty_invoiced, item.qty_invoiced) * IFNULL(item2.base_price, item.base_price) - ABS(IFNULL(item2.base_discount_amount, item.base_discount_amount) ) - ABS(COALESCE(awpto.points_to_money,0)/child_soi.total_item) ))"))
            ->columns(array('base_row_xinvoiced_incl_tax' => "IF(( IFNULL(item2.qty_invoiced, item.qty_invoiced) * IFNULL(item2.base_price, item.base_price) - ABS( IFNULL(item2.base_discount_amount, item.base_discount_amount) )  + IFNULL(item2.base_tax_invoiced, item.base_tax_invoiced) ) < 0, 0, ( IFNULL(item2.qty_invoiced, item.qty_invoiced) * IFNULL(item2.base_price, item.base_price) - ABS( IFNULL(item2.base_discount_amount, item.base_discount_amount) )  + IFNULL(item2.base_tax_invoiced, item.base_tax_invoiced) - ABS(COALESCE(awpto.points_to_money,0)/child_soi.total_item) ))"))
        # refunded
            ...
            ->columns(array('customer_email' => 'customer_email'));
    ...
    public function addOrderItems($skuType)
    {
        ...
        $this->getSelect()
            ...
            ->order("{$tableOrder}.{$filterField} DESC");
        return $this;
    }
    ...
    public function addCustomerInfo()
    {
        ...
        $this->getSelect()
            ->joinLeft(array('c_entity' => $customerEntity), "{$tableAlias}.customer_id = c_entity.entity_id", array('cust_email' => "c_entity.email"))
