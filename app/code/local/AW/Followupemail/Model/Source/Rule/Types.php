...
class AW_Followupemail_Model_Source_Rule_Types {
    ...
    const RULE_TYPE_PAYMENT_METHOD = 'payment_method_'; //adding payment method
    ...
    public static function toShortOptionArray($extended = FALSE) {
        ...
        //Get Payment Method
        $paymentMethods = Mage::getModel('sales/order_payment')->getCollection();		   
			$paymentMethods->join(array('payment'=>'sales/order_payment'),'main_table.entity_id=payment.parent_id','method');
			$paymentMethods->getSelect()
			//->distinct(true)
		        ->reset(Zend_Db_Select::COLUMNS)				 //buang semua kolom/field
				->columns('DISTINCT(payment.method) as method'); //di-distinct dan hanya menampilkan kolom method saja

        foreach ($paymentMethods as $code => $name){
			$paymentTitle = Mage::getStoreConfig('payment/'.$name["method"].'/title');		
            $result[self::RULE_TYPE_PAYMENT_METHOD . $name["method"]] = $helper->__("Order using '%s' payment method", $paymentTitle);
			}	
