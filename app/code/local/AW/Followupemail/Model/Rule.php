...
class AW_Followupemail_Model_Rule extends Mage_Core_Model_Abstract {
    ...
    public function prepareData() {
		...
		if ($len = strlen(AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_ORDER_STATUS_PREFIX)){
		...
		// $len = strlen(AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_PAYMENT_METHOD);
		else{
        $this->_orderStatus = (AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_PAYMENT_METHOD ==
                substr($this->getEventType(), 0, $len)) ? substr($this->getEventType(), $len) : false;
		}
		return true;
    }
    ...
    protected function _sendToSubscribersOnly($email) {
        if ($this->getSendToSubscribersOnly()) {
            ...
            if (!$subscriber || !$subscriber->getId() || Mage_Newsletter_Model_Subscriber::STATUS_SUBSCRIBED != $subscriber->getSubscriberStatus()
            ) {
                ...
                if (Mage::helper('followupemail')->canUseAN()) {
                    ...
                    if ($subscriber && $subscriber->getData() && $subscriber->getStatus() == AW_Advancednewsletter_Model_Subscriber::STATUS_SUBSCRIBED
                    )
                        return true;
                    return false;
                }
    ...
    protected function _validateByAdvancedNewsletterSegments($email) {
        if ($this->getSendToSubscribersOnly() && count($this->_anlSegments) && !in_array(AW_Followupemail_Model_Mysql4_Rule::ADVANCED_NEWSLETTER_SEGMENTS_ALL, $this->_anlSegments) && Mage::helper('followupemail')->canUseAN()
        ) {
            $subscriber = Mage::getModel('advancednewsletter/api')->getSubscriber($email);
    ...
    protected function _validateByCustomerGroup($customer) {
        if (!count($this->_customerGroups) || in_array(AW_Followupemail_Model_Source_Customer_Group::CUSTOMER_GROUP_ALL, $this->_customerGroups)
        )
        return true;
    ...
    public function validateByCustomer($params) {
        ...
        if (true !== $result = $this->_validateByCustomerGroup(isset($params['customer']) ? $params['customer'] : false))
            return $result;

        if (count($this->_stores) && isset($params['store_id']) && $params['store_id'] && !in_array($params['store_id'], $this->_stores)
        )
            return 'Wrong store';
    ...
    public function validateByProduct($params) {
        ...
        if (count($this->_productTypes) && !in_array(AW_Followupemail_Model_Source_Product_Types::PRODUCT_TYPE_ALL, $this->_productTypes) && isset($params['product_type_ids']) && count($params['product_type_ids'])
        ) {
        ...
        // category rule
        if (count($this->_categories) && isset($params['category_ids']) && count($params['category_ids'])
        ) {
        ...
        if (count($this->_productTypes) && !in_array(AW_Followupemail_Model_Source_Product_Types::PRODUCT_TYPE_ALL, $this->_productTypes) && isset($params['product_type_ids']) && count($params['product_type_ids']) && count($this->_sku) && isset($params['sku']) && count($params['sku'])
        ) {
            ...
            for ($i = 0; $i < count($params['product_type_ids']); $i++) {
                ...
                if (!is_null($_product['type']) && !is_null($_product['sku']) && in_array($_product['type'], $this->_productTypes) && in_array($_product['sku'], $this->_sku)
                ) {
    ...
    protected function _getTemplate($templateId, $storeId = null) {
        ...
        $templateName = substr($templateId, false !== ($pos = strpos($templateId, AW_Followupemail_Model_Source_Rule_Template::TEMPLATE_SOURCE_SEPARATOR)) ? $pos + 1 : 0);
    ...
    public function processBirthday($params, $templateId, $timeDelay, $hourDelay, $sequenceNumber) {
        ...
        if ($this->_isValid) {
			...
			//--------queue tidak berulang---------------------------------------------------------------------
									
			$resource = Mage::getSingleton('core/resource');
			$read = $resource->getConnection('core_read');
			
			$select = $read->select()
                ->from(array('afr' => $resource->getTableName('followupemail/rule')), array(
                    'id' => 'afr.id'))				
				->joinInner(array('afq' => $resource->getTableName('followupemail/queue')),'afq.rule_id=afr.id AND afq.`status`="R"', array(
					'queue_id' => 'afq.id',
					'status' => 'afq.status',
				))
                ->where('afq.recipient_email = ?', $objects['customer_email'])
				->where("afr.id = ".$this->getId())
                ->group('afr.id')
                ->order('afq.created_at');			
			$births = $read->fetchRow($select);
			//Zend_Debug::Dump($births); die;
			if ($births["status"] == "R"){
					return true;
				}			
            ...
            //Zend_Debug::Dump(strtotime(date("Y-m-d H:i:s")));
            $queue->add(
                    $code, $sequenceNumber, $content['sender_name'], $content['sender_email'], $objects['customer_name'], ($this->_isTest) ? $this->getTestRecipient() : $objects['customer_email'], //$customerEmail, 
                    $this->getId(),
                    //time(),
                    $scheduleAt, $content['subject'], $content['content'], $params['object_id'], $storedParams
            );
            return true;
        }		
    ...
    public function sendTestEmail($params, $objects = array()) {
        ...
        if (AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_CUSTOMER_BIRTHDAY == $this->getEventType()) {
            ...
            foreach (unserialize($this->getChain()) as $chain) {
                $result = $result && $this->processBirthday($params, $chain['TEMPLATE_ID'], $chain['DAYS'], $chain['HOURS'], $sequenceNumber);
                $sequenceNumber++;
            }
        }
        else
