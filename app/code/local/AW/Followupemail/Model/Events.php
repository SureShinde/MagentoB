        ...
        if (count($ruleIds)) {
            ...
            $objects = array(
             ...
             foreach ($ruleIds as $ruleId) {
                ...
                //agar tidak double masuk ke queue
                $query = "SELECT * FROM aw_followup_queue awq WHERE awq.recipient_email = '".$params["customer_email"]."' and awq.rule_id = '".$ruleId."' ";
                $queue = $read->fetchRow($query);

                if (!isset($queue["id"]))
                {
                //echo $ruleId."||".$objects["object_id"];
