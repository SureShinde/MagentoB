...
class Mage_SalesRule_Model_Resource_Rule extends Mage_Rule_Model_Resource_Abstract
{
    ...
    public function updateRuleTimesUsed($ruleId)
    {
        $query = sprintf("UPDATE %s SET times_used = times_used + 1 WHERE rule_id=:rule_id", $this->getMainTable());
        $binds = array("rule_id" => $ruleId);
        $this->_getWriteAdapter()->query($query, $binds);
    }
}
