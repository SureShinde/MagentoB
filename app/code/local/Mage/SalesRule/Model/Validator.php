...
class Mage_SalesRule_Model_Validator extends Mage_Core_Model_Abstract
{
     ...
     * @return  Mage_SalesRule_Model_Validator
     */
    public function process(Mage_Sales_Model_Quote_Item_Abstract $item, $rule, $xItem=NULL)
    {
...
//if(!isset($xItem[$item->getId()])) $xItem[$item->getId()]["discountAmount"] = 0;
...
/*echo '<br/>xItem discountAmount '.$xItem[$item->getId()]["discountAmount"];                    
echo '<br/>xItem baseDiscountAmount '.$xItem[$item->getId()]["baseDiscountAmount"];                    
*/                    $discountAmount    = ($qty*$itemPrice - $xItem[$item->getId()]["discountAmount"]) * $_rulePct;
                    $baseDiscountAmount= ($qty*$baseItemPrice - $xItem[$item->getId()]["baseDiscountAmount"]) * $_rulePct;
...
//echo '<br/>Validator->DiscountAmount-> '.$discountAmount; 
//echo '<br/>Validator->BaseDiscountAmount-> '.$baseDiscountAmount; 
            $item->setOriginalDiscountAmount($originalDiscountAmount);
            $item->setBaseOriginalDiscountAmount($baseOriginalDiscountAmount);
//echo '<br/>Validator->originalDiscountAmount-> '.$originalDiscountAmount; 
//echo '<br/>Validator->baseOriginalDiscountAmount-> '.$baseOriginalDiscountAmount; 
