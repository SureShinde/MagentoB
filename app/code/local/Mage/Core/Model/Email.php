...
class Mage_Core_Model_Email extends Varien_Object
{
    ...
    public function send()
    {
        ...
        if ($this->check_bounced_email()) {
            $mail->setFrom($this->getFromEmail(), $this->getFromName())
                ->addTo($this->getToEmail(), $this->getToName())
                ->setSubject($this->getSubject());
            $mail->send();
            Mage::log('Success send email to ' . $this->getToEmail() . '.');
        } else {
            Mage::log('Prevent sending email to ' . $this->getToEmail() . ', because the email address previously bounced.');
        }

        return $this;
    }

    public function check_bounced_email()
    {
        $resource = Mage::getSingleton('core/resource');
        $read = $resource->getConnection('core_read');
        $sql = sprintf("SELECT `email` FROM `bounced_email` WHERE `email` = '%s' LIMIT 1", $this->getToEmail());
