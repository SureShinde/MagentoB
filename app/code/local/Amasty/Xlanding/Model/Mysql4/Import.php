...
class Amasty_Xlanding_Model_Mysql4_Import extends Mage_Core_Model_Mysql4_Abstract
{
	...
	public function uploadAndImport(Varien_Object $object)
    {
        ...
        try {
            ...
            $io->streamClose();
            Mage::logException($e);
            Mage::throwException(Mage::helper('amlanding')->__('An error occurred while import Landing Pages.'));
        }
    ...
    public function validateCSVRow($line, $rowNumber)
    {
    	...
    	if (isset($row['attributes']) && $row['attributes'] != '' && @unserialize($row['attributes']) === FALSE) {
    		$this->_importErrors[] = Mage::helper('amlanding')->__('Attributes have invalid format. Check that they can be unseralizes using PHP function unserialize. Row #%d', $rowNumber);
    		return false;
    	} 
